...
/*
plaid-server.js – Configures the Plaid client and uses Express to defines routes that call Plaid endpoints in the Sandbox environment.
...
app.get('/api/reset_login', async (req, res, next) => {
  const access_token = req.session.access_token;
  try {
    const resetResponse = await client.sandboxItemResetLogin({ access_token });
...
app.get('/api/errors', async (req, res, next) => {
  return res.json({ error: currentError });
});

function handleItemWebhook(code, requestBody) {
  switch (code) {
    case 'ERROR':
      console.log(requestBody.error.error_message);
      currentError = requestBody;
