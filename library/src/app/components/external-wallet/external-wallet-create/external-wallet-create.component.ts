...
@Component({
  selector: 'app-external-wallet-create',
  templateUrl: './external-wallet-create.component.html',
  styleUrls: ['./external-wallet-create.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ExternalWalletCreateComponent implements OnInit, OnDestroy {
  ...
  listAccounts(): void {
    this.accountService
      ...
      .pipe(
        tap((accounts) => {
          let tradingAccounts = accounts.objects.filter(
  ...
  onCreateWallet(): void {
    ...
    const name = <string>(
      this.newWalletFormGroup.controls.name.value?.toString()
    );
    const address = <string>(
      this.newWalletFormGroup.controls.address.value?.toString()
    );
    const tag = <string>this.newWalletFormGroup.controls.tag.value?.toString();
    if (!this.validateForm(name, address)) {
      return;
    }
  ...
  createPostExternalWalletBankModel(
    asset: string,
    name: string,
    address: string,
